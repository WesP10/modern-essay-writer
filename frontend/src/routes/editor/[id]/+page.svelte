<script lang="ts">
  ...
  import GeneratorModal from "$lib/components/ai/GeneratorModal.svelte";
  ...
  // Auto-save when title changes (after initial load)
  $: if (initialLoadComplete && title) {
    handleTitleChange();
  }
  ...
  function handleGeneratorInsert(e: CustomEvent<string>) {
    const text = e.detail;
    if (text && editor) {
...
>
  ...
  <div class="flex-1 overflow-hidden">
    <ResizablePanels {showSidebar}>
      ...
      <svelte:fragment slot="sidebar">
        <SidebarContainer onClose={toggleSidebar} {editor} />
      </svelte:fragment>
    </ResizablePanels>
  </div>
  ...
  <GeneratorModal
    bind:isOpen={showGeneratorModal}
    on:insert={handleGeneratorInsert}
  />
